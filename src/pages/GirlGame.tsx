    ...
    const handleGameEnd = async (e: Event) => {
      ...
      if (playerName) {
        try {
          ...
          } else if (finalScore < existing.selections_used) {
            // New score is better, update existing record
            await supabase.from('girl_game_scores')
              .update({ selections_used: finalScore, created_at: new Date().toISOString() })
              .eq('id', existing.id);
          }
        } catch (error) {
